...
const ComparisonChart: React.FC<ComparisonChartProps> = ({ summaries, isTeamView, simDate }) => {
  ...
  return (
    <div className="bg-white p-4 md:p-8">
      <h2 className="text-xs md:text-base font-black text-gray-900 uppercase tracking-widest mb-6 md:mb-10 flex items-center gap-2 md:gap-3">
        <span className={`w-1 md:w-1.5 h-5 md:h-6 rounded-full ${isTeamView ? 'bg-black' : 'bg-red-600'}`}></span>
      ...
      <div className="h-64 md:h-96 w-full">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData} margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
            ...
            {/* Fixed: removed md:fontSize which is invalid JSX prop syntax here */}
            ...
            <Tooltip 
              contentStyle={{ border: 'none', borderRadius: '16px', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)', padding: '12px', fontSize: '10px' }}
              itemStyle={{ fontWeight: 800, textTransform: 'uppercase' }}
              labelStyle={{ fontWeight: 900, marginBottom: '6px', color: '#000' }}
            />
            
            <Legend 
              verticalAlign="bottom" 
              iconType="circle"
              /* Fixed: removed invalid md:fontSize syntax in object literal */
              wrapperStyle={{ fontSize: '9px', textTransform: 'uppercase', fontWeight: 800, paddingTop: '20px' }} 
            />
            ...
            {simDayFormatted && (
              <ReferenceLine 
                ...
                strokeDasharray="4 4" 
                /* Fixed: removed invalid md:fontSize syntax in object literal */
                label={{ position: 'top', value: 'NOW', fontSize: 8, fill: '#dc2626', fontWeight: 900 }} 
              />
            )}
            ...
            {summaries.map(summary => (
              <Line 
                key={summary.id}
                type="monotone" 
                dataKey={summary.name} 
                stroke={(COLORS as any)[summary.id] || '#111'} 
                strokeWidth={3} 
                dot={false}
